...
class Upload
{
    ...
    public function upload($files = '')
    {
        ...
        foreach ($files as $key => $file) {
            ...
            if ($this->uploader->save($file, $this->replace)) {
                ...
                $file['filepath'] = $this->rootPath . $file['savepath'] . $file['savename'];//addtime 2016年3月9日22:36:09
                $info[$key] = $file;
            } else {
                $this->error = $this->uploader->getError();
            }
    ...
    private function setDriver($driver = null, $config = null)
    {
        ...
        $configUpload = $configObj->upload->toArray();
        $driver = $driver ?: ($this->driver ?: $configUpload['upload_type']);
        $config = $config ?: ($this->driverConfig ?: $configUpload['config']);
        $class = strpos($driver, '\\') ? $driver : '\\Core\\Upload\\' . ucfirst(strtolower($driver));
